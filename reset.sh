...
if [ -d "$root/finale" ]; then
  ...
  echo "# Put the final password here (see evidence)" > "$root/finale_locked/password.txt"
fi
...
if [ -d "$root/evidence" ]; then
  ...
  echo "# Put the second password here (see suspects clues)" > "$root/evidence_locked/password.txt"
fi
...
if [ -d "$root/suspects" ]; then
  ...
  echo "# Put the first password here (see crime_scene)" > "$root/suspects_locked/password.txt"
fi
